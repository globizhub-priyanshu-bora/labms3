...
const labSchema = p.pgTable("labSchema", {
  ...
  addressLine1: p.text(),
  logo: p.text(),
  photoDocument: p.text(),
  // NEW: Lab photo document (registration proof)
...
const doctorSchema = p.pgTable("doctorSchema", {
  ...
  name: p.text().notNull(),
  gender: p.text(),
  // NEW: Gender field (Male, Female, Other)
  specialization: p.text(),
  phoneNumber: p.bigint({ mode: "number" }),
  photoDocument: p.text(),
  // NEW: Photo/ID document upload
  createdAt: p.timestamp().defaultNow(),
  updatedAt: p.timestamp().defaultNow(),
  deletedAt: p.timestamp()
});
...
const reportSchema = p.pgTable("reportSchema", {
  id: p.serial().primaryKey(),
  labId: p.integer().references(() => labSchema.id).notNull(),
  patientTestId: p.integer().references(() => patientTestsSchema.id).notNull(),
  billId: p.integer().references(() => billSchema.id),
...
const db = drizzle(process.env.DATABASE_URL || "postgresql://neondb_owner:npg_dOYs1VeNkE6B@ep-gentle-rice-a1voph97-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require", { schema });
...
export {
  patientTestsSchema as a,
  billSchema as b,
  doctorSchema as c,
  db as d,
  testParamSchema as e,
  testResultsSchema as f,
